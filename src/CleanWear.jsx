...
import QuickDetective from "./QuickDetective.jsx";
...
const FD_Q = [
  { id: "stretch", q: "Stretch Test", inst: "Pull the fabric sideways with both hands, then let go.", opts: [{ label: "Snaps right back", value: "elastic", icon: "\ud83d\udd04" }, { label: "Stretches a little, slowly returns", value: "slight", icon: "\u2194\ufe0f" }, { label: "Barely stretches at all", value: "none", icon: "\ud83e\uddf1" }] },
  { id: "feel", q: "Texture Test", inst: "Rub the fabric between your thumb and fingers.", opts: [{ label: "Slick, smooth, almost plasticky", value: "slick", icon: "\u2728" }, { label: "Soft, matte, natural feeling", value: "soft", icon: "\u2601\ufe0f" }, { label: "Rough, textured, slightly scratchy", value: "rough", icon: "\ud83e\udea8" }, { label: "Fuzzy, warm, woolly", value: "fuzzy", icon: "\ud83d\udc11" }] },
  { id: "wrinkle", q: "Wrinkle Test", inst: "Scrunch it in your fist for 5 seconds, then release.", opts: [{ label: "Springs back smooth", value: "none", icon: "\u2705" }, { label: "Some wrinkles, mostly recovers", value: "some", icon: "\u3030\ufe0f" }, { label: "Stays wrinkled", value: "lots", icon: "\ud83d\udcc3" }] },
  { id: "sheen", q: "Sheen Test", inst: "Hold the fabric under a light and tilt it.", opts: [{ label: "Shiny / reflective", value: "shiny", icon: "\ud83d\udc8e" }, { label: "Completely matte", value: "matte", icon: "\u2b1b" }] },
  { id: "temp", q: "Temperature Test", inst: "Press the fabric against your forearm for 3 seconds.", opts: [{ label: "Feels cool or cold", value: "cool", icon: "\u2744\ufe0f" }, { label: "Feels warm or neutral", value: "warm", icon: "\ud83d\udd25" }] },
  { id: "use", q: "What type of garment?", inst: "Helps narrow down likely materials.", opts: [{ label: "Athletic / sportswear", value: "athletic", icon: "\ud83c\udfc3" }, { label: "Casual everyday", value: "casual", icon: "\ud83d\udc55" }, { label: "Dress / formal", value: "formal", icon: "\ud83d\udc54" }, { label: "Underwear / base layer", value: "underwear", icon: "\ud83e\ude72" }, { label: "Sleepwear / lounge", value: "sleep", icon: "\ud83d\ude34" }] },
];
...
function predictFabric(a) {
  ...
  const chemMap = { polyester: ["Antimony trioxide", "Microplastics", "BPA/BPS"], nylon: ["Microplastics", "Formaldehyde resins"], cotton: ["Formaldehyde (wrinkle treatment)", "Pesticide residues"], cotton_poly: ["Antimony", "Microplastics", "Formaldehyde"], linen: ["Minimal \u2014 very low risk"], wool: ["Minimal \u2014 very low risk"], rayon: ["Carbon disulfide", "Sodium hydroxide"] };
...
export default function CleanWearApp() {
  ...
  useEffect(() => { if (wardrobe.length > 0) { localStorage.setItem("cw_wardrobe", JSON.stringify(wardrobe)); analytics.syncWardrobeProfile(wardrobe); } }, [wardrobe]);
  useEffect(() => { const i = setInterval(() => setFactIdx(x => (x + 1) % FUN_FACTS.length), 7000); return () => clearInterval(i); }, []);
  useEffect(() => { const handlePop = () => { if (view === "results") { setView("scanner"); setResult(null); } }; window.addEventListener("popstate", handlePop); return () => window.removeEventListener("popstate", handlePop); }, [view]);
  ...
  const addWard = useCallback(() => { if (!result || !score) return; setWardrobe(p => [{ id: Date.now(), name: result.product_name, brand: result.brand, category: result.category, score: score.overall, at: new Date().toISOString() }, ...p]); setAdded(true); analytics.trackWardrobeAdd(result.product_name, result.brand, score.overall); }, [result, score]);
  const rmWard = useCallback((id) => { setWardrobe(p => { const item = p.find(i => i.id === id); if (item) analytics.trackWardrobeRemove(item.name, item.brand, item.score); const u = p.filter(i => i.id !== id); if (!u.length) localStorage.removeItem("cw_wardrobe"); return u; }); }, []);
  const avg = wardrobe.length ? Math.round(wardrobe.reduce((s, i) => s + i.score, 0) / wardrobe.length) : 0;

  const renderScanner = () => (<>
    <div className="hero"><div className="hero-eyebrow">Textile Safety Intelligence</div><h1>Your Clothes Are <em style={{fontFamily:'var(--sans)'}}>Quietly</em> Hurting You</h1><div className="hero-sub">CleanWear scans your clothing for hidden carcinogens, endocrine disruptors, and toxic chemicals — so you can protect your body from what you wear every day.</div></div>
    ...
    <div className="scan-area"><div className="scan-label">Scan Your Clothing</div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <button onClick={() => setShowCamera(true)} style={{ flex: 1, padding: "16px 12px", background: "linear-gradient(135deg, rgba(74,222,128,.08), rgba(74,222,128,.03))", border: "1.5px solid rgba(74,222,128,.2)", borderRadius: 16, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, fontFamily: "var(--sans)", transition: "all .25s" }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: "var(--g4)" }}>Open Camera</span>
    ...
    <div className="fd-sec"><div className="fd-label">No Tag? No Problem</div>{!fdActive ? (<><div className="fd-sub">Don't know the brand or materials? Answer 3 quick questions about how it feels — get an instant safety score.</div><button className="fd-start" onClick={() => { setFdActive(true); analytics.trackFabricDetectiveStarted(); }}>{"\ud83d\udd0d"} Start Fabric Detective</button></>) : (<QuickDetective onFullScan={(matQuery) => { setQuery(matQuery); scanSourceRef.current = "fabric_detective"; analytics.trackFabricDetectiveFullAnalysis(); doScan(matQuery); setFdActive(false); }} onClose={() => setFdActive(false)} />)}</div>
        ...
        onShare={() => {
          if (navigator.share) {
            navigator.share({
              ...
              text: `I just scanned my ${result.brand} ${result.product_name} — it scored ${score.overall}/100 for chemical safety. Check yours at cleanwear.app`,
              url: "https://cleanwear.app",
            }).catch(() => {});
          } else {
            navigator.clipboard?.writeText(
              `My ${result.brand} ${result.product_name} scored ${score.overall}/100 on CleanWear. Scan yours: https://cleanwear.app`
            );
          }
        }}
      />
  ...
  const renderWardrobe = () => (<><div className="w-hero"><h1 style={{ fontFamily: "var(--serif)", fontSize: 24, fontWeight: 700, marginBottom: 20 }}>My Wardrobe</h1>{wardrobe.length > 0 && (<><div className="w-agg" style={{ borderColor: sc(avg) }}><div className="w-an" style={{ color: sc(avg) }}>{avg}</div><div className="w-al">Avg Score</div></div><p style={{ fontSize: 13, color: "var(--tx3)" }}>{wardrobe.length} item{wardrobe.length !== 1 ? "s" : ""} {"\u00b7"} {sg(avg)} overall</p></>)}</div>{!wardrobe.length ? <div className="w-empty"><div style={{ fontSize: 44, marginBottom: 16, opacity: .4 }}>{"\ud83d\udc55"}</div><p style={{ fontSize: 15, marginBottom: 8, fontWeight: 500 }}>Your wardrobe is empty</p><p style={{ fontSize: 13 }}>Scan items to build your health profile.</p></div> : (<><div style={{ padding: "0 24px", marginBottom: 16 }}><div style={{ fontSize: 10, fontWeight: 700, color: "var(--tx4)", marginBottom: 8, letterSpacing: "1.5px", textTransform: "uppercase" }}>This Week</div><div className="streak">{["M","T","W","T","F","S","S"].map((d, i) => { const h = wardrobe.some(w => new Date(w.at).getDay() === (i + 1) % 7); return <div key={i} className="streak-d" style={{ background: h ? "var(--s2)" : "var(--s1)", color: h ? "var(--g4)" : "var(--tx4)", border: `1px solid ${h ? "var(--g8)" : "var(--bd)"}` }}>{d}</div>; })}</div></div>{wardrobe.map(w => (<div key={w.id} className="w-item" onClick={() => { setQuery(`${w.brand} ${w.name}`); scanSourceRef.current = "wardrobe_rescan"; doScan(`${w.brand} ${w.name}`); }}><div className="w-is" style={{ background: `${sc(w.score)}14`, color: sc(w.score), border: `1px solid ${sc(w.score)}33` }}>{w.score}</div><div className="w-ii"><div className="w-in">{w.name}</div><div className="w-ib">{w.brand} {"\u00b7"} {w.category}</div></div><button className="w-ir" onClick={e => { e.stopPropagation(); rmWard(w.id); }}>{"\u2715"}</button></div>))}</>)}</>);
