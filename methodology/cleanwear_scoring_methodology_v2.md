# CleanWear Scoring Methodology v3
...
```
IF category = "Kids" AND category subtype = "Sleepwear"
   AND finish_claims does NOT contain "fitted" or "snug-fit"
THEN flag Flame Retardants at HIGH confidence
   [US CPSC 16 CFR 1615/1616 requires flame resistance in non-fitted
    children's sleepwear. Compliance is almost universally achieved through
    chemical treatment in synthetic garments.]

IF category = "Sleepwear" (adult)
...
**Step 3 — Apply certification bonus.** Each recognized certification (§B.2) adds +8 points, capped at +20 total. The bonus represents reduced uncertainty, not confirmed safety.
...
When multiple signals are available for a brand, use the most specific and most recent.
...
```json
{
  ...
  "components": [
    ...
    },
    {
      ...
      "score": 52,
      "weight": 0.35,
      "source": "NRDC PFAS Scorecard 2023",
...
This trace must be stored in the `scans` table alongside the score (the `chemicals` JSONB column can hold this structure). It must be re-producible: running the same inputs through the engine must always produce the same trace.
...
| 2 | NRDC PFAS Brand Scorecard | Annual PDF report; manual ingest | Annual | Brand C2 ratings | **Operational (manual, annual refresh)** |
...
### Example 1: Lululemon Align Leggings (High-Risk Athleisure)
...
**C2 (Brand Safety):** Lululemon → NRDC PFAS rating C → **52**
...
**C2 (Brand Safety):** Uniqlo → Good On You "It's a Start" → **48**
...
**C1:** 100 − 5 + 8 = **103 → clamped to 100** (max)

Wait — let me re-examine. The cert bonus is capped at +20 and adds to the base after penalty deduction. 100 − 5 = 95, + 8 = 103 → clamped at 100. So **C1 = 100**.

**C2 (Brand Safety):** Patagonia → bluesign_certified AND Good On You "Great" → use most favorable: Good On You "Great" → **82**

**C3 (Category Benchmark):** Athletic (synthetic, no DWR claim) → **48**

**Final score:** (100 × 0.45) + (82 × 0.35) + (48 × 0.20) = 45.00 + 28.70 + 9.60 = **83** (rounded)

**Confidence:** Tier 2 — "Strong Evidence"

**Disclosure notes:** Low-confidence flags only: Microplastics (recycled polyester, shedding not addressed by any cert).

**Score interpretation:** 83/100, Lower Risk. The bluesign certification suppresses the phthalates, antimony, and azo dye concerns. The remaining microplastics flag is real and disclosed but carries a low scoring penalty. The brand's strong public safety record is the second major positive driver.
...
- Polyester flagged for antimony at MEDIUM confidence will include products that were manufactured with low-antimony catalysts or that have been certified but where the cert information has not yet been ingested into our registry.

**Known false negatives:**

- A garment from an uncertified brand with no public safety record will receive a LOW-data score. If that brand's products actually contain high chemical levels, the score will be underestimating risk.
...
- Finish treatments declared as "stain-resistant" cover a wide range of chemistries; not all use fluoropolymers. Our PFAS flag for stain-resistant claims at MEDIUM confidence will over-flag non-PFAS stain treatments.
...
- NRDC PFAS scorecard data is from 2023. Brands that have changed practices since then are incorrectly scored until the next annual update.

---

*Document prepared 2026-04-24. This methodology replaces all prior scoring descriptions. Any marketing or UI copy that describes CleanWear's scoring must be consistent with this document. Any discrepancy between UI copy and this document should be resolved in favor of this document.*
