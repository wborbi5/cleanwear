...
**Prior version:** `methodology/cleanwear_scoring_methodology_v2.md` (retained for version history)
...
3. **Independent third-party brand rating with the most recent publication date.** If NRDC 2023 and Good On You 2024 both exist, use Good On You 2024.
4. **Older independent brand ratings** (data older than 2 years at time of scoring) are disclosed in the score trace but carry a one-tier confidence downgrade to the overall score confidence.
...
> *Priority note: NRDC 2023 is the only independent brand rating on record for TNF. If a bluesign brand-level approval exists for TNF generally, it would be priority 2 and would score 75, substantially changing the result. The trace must record which signal was used and when it was last verified.*
