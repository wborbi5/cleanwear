  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", paddingBottom: 120 }}>
      ...
      <div style={{ padding: "20px 24px 0" }}>
        <button className="bk-btn" onClick={onBack}>
          ← Back
        </button>
      </div>
      ...
      <div style={{ padding: "0 24px 24px" }}>
        <div style={{ fontSize: 10, letterSpacing: "2.5px", textTransform: "uppercase", color: "var(--gold)", fontWeight: 700, marginBottom: 14 }}>
          What you can do now
        </div>
        <div style={{ background: "var(--s1)", border: "1px solid var(--bd)", borderRadius: 18, overflow: "hidden" }}>
          {[
            { icon: "🏷️", title: "Check the care label", desc: "Look for fabric composition — 100% cotton or organic cotton is a safer starting point than polyester blends." },
            { icon: "✅", title: "Look for certifications", desc: "OEKO-TEX Standard 100, GOTS, or bluesign® badges on the tag mean the item has been tested for harmful substances." },
            { icon: "🌿", title: "Check Good On You", desc: "Good On You rates brands on environment, labor, and animal welfare. Search your brand at goodonyou.eco." },
          ].map((tip, i, arr) => (
            <div key={i} style={{
              display: "flex",
              gap: 14,
              ...
              <div style={{ fontSize: 22, marginTop: 1, minWidth: 28, textAlign: "center" }}>{tip.icon}</div>
              <div>
                <div style={{ fontWeight: 700, fontSize: 13, color: "var(--tx)", marginBottom: 4 }}>{tip.title}</div>
                <div style={{ fontSize: 13, color: "var(--tx3)", lineHeight: 1.6 }}>{tip.desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ padding: "0 24px 20px" }}>
        <div style={{ fontSize: 10, letterSpacing: "2.5px", textTransform: "uppercase", color: "var(--tx4)", fontWeight: 700, marginBottom: 14 }}>
          Try a known brand instead
        </div>
        <div style={{ position: "relative", marginBottom: 14 }}>
          <input
            ...
            onKeyDown={e => { if (e.key === "Enter" && retryQuery.trim()) onSearchNew(retryQuery.trim()); }}
            placeholder='e.g. "Nike Dri-FIT Tee" or "Lululemon Align"'
            style={{
              width: "100%",
          ...
          />
          <button
            onClick={() => { if (retryQuery.trim()) onSearchNew(retryQuery.trim()); }}
            style={{
        ...
        {/* Quick chips for well-known brands */}
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {["Nike Tee", "Lululemon Align", "Patagonia Fleece", "Uniqlo Heattech", "Gymshark Shorts"].map(chip => (
            <button
              key={chip}
              onClick={() => onSearchNew(chip)}
              style={{
                padding: "9px 16px",
                borderRadius: 24,
                background: "var(--s1)",
...
export default function CleanWearApp() {
  ...
  const [unknownQuery, setUnknownQuery] = useState(null);
  ...
  const doScan = useCallback(async (q, bc = false) => {
    ...
    try {
      ...
      // Product not in database — show research-request screen instead of a fabricated score
      if (pd._unknown) {
        setUnknownQuery(q);
        navigateToResults();
        window.posthog?.capture("scan_unknown_product", { query: q });
        return;
      }
  ...
  const renderScanner = () => (<>
    ...
    <div className="scan-area">
      ...
      {!user && (() => { const st = getScanStatus(user); return (<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 2px 0",fontSize:12,fontWeight:500,color:st.remaining<=1?"var(--r4)":"var(--tx4)"}}><span style={{display:"flex",alignItems:"center",gap:6}}><span style={{display:"inline-flex",gap:3}}>{[...Array(5)].map((_,i)=>(<span key={i} style={{width:7,height:7,borderRadius:"50%",background:i<st.remaining?"var(--g5)":"var(--bd2)",display:"inline-block"}}/>))}</span>{st.remaining} of 5 free scans left</span><button onClick={()=>{setAuthTrigger("scan_limit");setAuthModalOpen(true);}} style={{background:"none",border:"none",color:"var(--g6)",fontFamily:"var(--sans)",fontSize:12,fontWeight:600,cursor:"pointer",padding:0}}>Sign in for unlimited →</button></div>); })()}
      </div>
    ...
    <div className="works">
      <div className="works-label">What Works Best</div>
      <div className="works-card">
        <div className="works-row good"><span className="works-ic">{"\u2713"}</span><span>Major brands like Nike, Lululemon, Patagonia, Uniqlo, Gymshark, Under Armour</span></div>
        <div className="works-row good"><span className="works-ic">{"\u2713"}</span><span>Specific product names ("Align Leggings," "Dri-FIT Tee")</span></div>
        <div className="works-row good"><span className="works-ic">{"\u2713"}</span><span>UPC / EAN barcodes scanned from care tags</span></div>
        <div className="works-row warn"><span className="works-ic">{"\u26a0"}</span><span>Small or unknown brands may not score — we rely on public data</span></div>
      </div>
    </div>
  </>);
  ...
  const renderResults = () => {
    if (unknownQuery) {
      return (
        <UnknownProductView
          query={unknownQuery}
          onBack={() => { setUnknownQuery(null); window.history.back(); }}
          onSearchNew={(q) => { setUnknownQuery(null); setQuery(q); scanSourceRef.current = "unknown_retry"; doScan(q); }}
        />
      );
    }
    ...
    return (
      <ResultsPage
        ...
        onNavigateCertify={() => navigateTo("certify")}
      />
    );
  };

  const renderWardrobe = () => {
    // ── Logged-out: full sign-in page ────────────────────────
    if (!user && !authLoading) {
      return (
        <div style={{ padding: "0 0 120px" }}>
          <div style={{ padding: "48px 24px 32px", textAlign: "center" }}>
            <div style={{ fontFamily: "var(--serif)", fontSize: 26, fontWeight: 700, marginBottom: 8, color: "var(--tx)" }}>Your Clean<em style={{ fontStyle: "italic", color: "var(--g6)" }}>Wear</em> Profile</div>
            <p style={{ fontSize: 14, color: "var(--tx3)", lineHeight: 1.6, maxWidth: 300, margin: "0 auto" }}>
              Save scanned items, track your wardrobe safety score, and get weekly insights.
            </p>
          </div>
          <div style={{ padding: "0 24px", marginBottom: 24 }}>
            <InlineSignIn />
          </div>
          <div style={{ margin: "0 24px", padding: "20px 24px", background: "var(--s1)", border: "1px solid var(--bd)", borderRadius: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--tx4)", letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 14 }}>What you get</div>
            {[
              ["Unlimited scans", "Free accounts never hit a scan limit"],
              ["Wardrobe tracker", "Save items and see your household's avg safety score"],
              ["Weekly digest", "Your highest-risk items, ranked by exposure"],
            ].map(([title, desc]) => (
              <div key={title} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: "1px solid var(--bd)" }}>
                <span style={{ color: "var(--g6)", fontWeight: 700, fontSize: 14, marginTop: 1 }}>✓</span>
                <div>
            ...
            <div style={{ display: "flex", gap: 12, padding: "10px 0" }}>
              <span style={{ color: "var(--g6)", fontWeight: 700, fontSize: 14, marginTop: 1 }}>✓</span>
              <div>
                <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 2 }}>Free forever</div>
                <div style={{ fontSize: 12, color: "var(--tx3)" }}>No credit card, no paywall</div>
              </div>
            </div>
          </div>
        </div>
    ...
    if (authLoading) {
      return <div className="ld-c"><div className="ld-spin" /><div><div className="ld-t">Loading profile...</div></div></div>;
    }
    return (<><div style={{ padding: "20px 24px 0", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid var(--bd)", paddingBottom: 16, marginBottom: 4 }}><div><div style={{ fontSize: 12, color: "var(--tx4)", marginBottom: 2 }}>Signed in as</div><div style={{ fontWeight: 600, fontSize: 13, color: "var(--tx)" }}>{user?.email}</div></div><button onClick={async () => { await signOut(); }} style={{ padding: "8px 16px", background: "none", border: "1px solid var(--bd)", borderRadius: 10, fontFamily: "var(--sans)", fontSize: 12, fontWeight: 600, color: "var(--tx3)", cursor: "pointer" }}>Sign out</button></div><div className="w-hero"><h1 style={{ fontFamily: "var(--serif)", fontSize: 24, fontWeight: 700, marginBottom: 20 }}>My Wardrobe</h1>{wardrobe.length > 0 && (<><div className="w-agg" style={{ borderColor: sc(avg) }}><div className="w-an" style={{ color: sc(avg) }}>{avg}</div><div className="w-al">Avg Score</div></div><p style={{ fontSize: 13, color: "var(--tx3)" }}>{wardrobe.length} item{wardrobe.length !== 1 ? "s" : ""} {"\u00b7"} {sg(avg)} overall</p></>)}</div>{!wardrobe.length ? <div className="w-empty"><div style={{ fontSize: 44, marginBottom: 16, opacity: .4 }}>{"\ud83d\udc55"}</div><p style={{ fontSize: 15, marginBottom: 8, fontWeight: 500 }}>Your wardrobe is empty</p><p style={{ fontSize: 13 }}>Scan items to build your health profile.</p></div> : (<><div style={{ padding: "0 24px", marginBottom: 16 }}><div style={{ fontSize: 10, fontWeight: 700, color: "var(--tx4)", marginBottom: 8, letterSpacing: "1.5px", textTransform: "uppercase" }}>This Week</div><div className="streak">{["M","T","W","T","F","S","S"].map((d, i) => { const h = wardrobe.some(w => new Date(w.at).getDay() === (i + 1) % 7); return <div key={i} className="streak-d" style={{ background: h ? "var(--s2)" : "var(--s1)", color: h ? "var(--g6)" : "var(--tx4)", border: `1px solid ${h ? "var(--g8)" : "var(--bd)"}` }}>{d}</div>; })}</div></div>{wardrobe.map(w => (<div key={w.id} className="w-item" onClick={() => { setQuery(`${w.brand} ${w.name}`); scanSourceRef.current = "wardrobe_rescan"; doScan(`${w.brand} ${w.name}`); }}><div className="w-is" style={{ background: `${sc(w.score)}14`, color: sc(w.score), border: `1px solid ${sc(w.score)}33` }}>{w.score}</div><div className="w-ii"><div className="w-in">{w.name}</div><div className="w-ib">{w.brand} {"\u00b7"} {w.category}</div></div><button className="w-ir" onClick={e => { e.stopPropagation(); rmWard(w.id); }}>{"\u2715"}</button></div>))}</>)}</>);
  };
